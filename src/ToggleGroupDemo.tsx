...
import KnobPercentage, { KnobHeadless } from "react-knob-headless";
...
export function Sequencer({ samples, numOfSteps = 16 }: Props) {
  ...
  return (
    <>
      <div className="flex flex-col items-center space-y-2 bg-neutral-950 h-screen">
              ...
              <div className="flex flex-row gap-2 justify-center align-middle items-center w-full space-y-2">
                ...
                >
                  {stepIds.map((stepId, stepIndex) => {
                    ...
                    // every four steps, change the shade of the lamp ->
                    const shade = stepIndex % 4 === 0 ? 900 : 800;

                    return (
                      <label
                        key={id}
                        className={cn(
                          `w-12 h-10 rounded-sm flex items-center justify-center bg-neutral-${shade} transition-colors duration-100 scale-100 hover:scale-110 cursor-pointer`,
                          {
